...
describe('고정비 점검 카드', () => {
  it('고정지출이 없으면 카드를 아예 띄우지 않는다', () => {
    const { container } = render(<FixedCostCheck items={income500} closed />)
    expect(container).toBeEmptyDOMElement()
  })

  it('줄일 여지를 금액과 10년치로 보여준다', () => {
    // 수입 500만, 보험 70만 → 상한 50만, 20만 초과 → 10년 2,400만
    render(<FixedCostCheck items={[...income500, item('fixed', '보험', 700_000)]} closed />)
  ...
  it('다 괜찮으면 줄일 여지 안내를 띄우지 않는다', () => {
    render(<FixedCostCheck items={[...income500, item('fixed', '통신', 150_000)]} closed />)
  ...
  it('수입이 없으면 판단 대신 안내를 보여준다', () => {
    render(<FixedCostCheck items={[item('fixed', '보험', 500_000)]} closed />)
  ...
  it('기준의 출처를 펼쳐 볼 수 있다', async () => {
    ...
    render(<FixedCostCheck items={[...income500, item('fixed', '주거', 1_000_000)]} closed />)
