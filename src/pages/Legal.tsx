...
// 내용을 고칠 때마다 같이 올린다 — 방침이 바뀌었는데 시행일이 그대로면 안 된다.
// 2026-09-01: 호스팅 수탁자 Netlify → Cloudflare 정정, 국외 이전을 동의 대신 공개 방식으로
// 2026-09-02: 마케팅 정보 수신(선택) 항목 추가
// 2026-09-02: AI 코칭 기능 제거 — Anthropic은 맞춤 리포트 초안 다듬기 용도로만 남음
...
const BIZ = {
  name: '마이머니플랜', // 상호
  ceo: '김선숙', // 대표자
  regNo: '223-29-01904', // 사업자등록번호
  salesNo: '제2025-경기여주-00061호', // 통신판매업 신고번호
  address: '경기도 여주시 청심로 65, 4층 401-D03호(하동, 신화빌딩)', // 사업장 주소
  tel: '', // 연락처 (미기재 시 문의 이메일로 안내)
}
  ...
  return (
    <>
      ...
      <p>
        ...
        구성원에 한함), 이용자가 신청한 맞춤 리포트 작성, 서비스 개선·오류 대응.
      </p>
      <p>
        <b>[선택] 마케팅 정보 수신</b> — 별도로 동의한 이용자에 한해, 새 기능·재테크 소식·이벤트 안내를
        이메일로 보내드립니다. 동의하지 않아도 회원 가입 및 서비스 이용에 제한이 없으며, <b>설정 화면에서
        언제든 철회</b>할 수 있습니다. 철회하더라도 가입 확인·중요 공지 등 서비스 운영에 필요한 안내
        메일은 계속 발송됩니다. 보유 기간: 동의 철회 또는 회원 탈퇴 시까지.
      </p>
      ...
      <p>
        ...
        저장·처리됩니다. 이는 <b>계약의 이행 및 이용자 편의 증진을 위하여 필요한 처리위탁·보관</b>에
        해당하므로, 「개인정보 보호법」 제28조의8 제1항 제3호에 따라 별도의 동의 없이 아래와 같이
        공개합니다.
      </p>
      <p>
        · <b>Supabase, Inc.</b> (미국) — 데이터베이스·인증(계정·가계부 데이터 저장). 이전 방법: 서비스
        이용 시 정보통신망을 통한 전송. 이전 시점: 서비스 이용 시. 이전 항목: 위 1항의 정보.
        보유·이용 기간: 회원 탈퇴 시까지.
        <br />· 호스팅: <b>Cloudflare, Inc.</b>(미국) — 웹사이트 파일 전송·보안. 이전 방법·시점: 접속 시
        정보통신망을 통한 전송. 이전 항목: 접속 기록(IP 주소 등). 보유·이용 기간: 서비스 제공 기간.
        <br />· <b>Anthropic, PBC</b> (미국) — <b>이용자가 맞춤 리포트를 신청한 경우에 한해</b>, 리포트
        초안의 문장을 다듬는 데 이용됩니다. 이전 방법·시점: 리포트 작성 시 정보통신망을 통한 전송.
        이전 항목: 리포트 초안에 담긴 구성원 호칭과 금액, 신청 시 적은 문의 내용. 이메일·비밀번호·계좌
        정보는 전송하지 않습니다. 보유·이용 기간: 처리 즉시 파기(모델 학습에 이용되지 않음).
      </p>
      <p>
        이용자는 개인정보의 국외 이전을 거부할 수 있습니다. 다만 서비스가 국외 클라우드를 기반으로
        운영되는 특성상, 거부 시 회원 가입 및 서비스 이용이 제한됩니다. 거부를 원하시면 아래 연락처로
        요청해 주세요.
      </p>
  ...
  return (
    <>
      ...
      <p>
        ...
        및 캐릭터 반응 등을 제공합니다. 기본 기능은 무료로 제공되며, 일부 유료 서비스(예: 맞춤 리포트)가
        추가될 수 있습니다. 기능은 사전 고지 후 변경·중단될 수 있습니다.
      </p>
      ...
      <p>
        유료 서비스의 결제는 운영자가 지정한 전자결제대행사(PG) 또는 외부 판매 플랫폼(예: 오픈마켓)을
        통해 이루어지며, 가격·결제 주기·제공 내용은 구매 시점 화면에 고지합니다. 외부 플랫폼을 통해
        구매한 경우 해당 플랫폼의 결제·환불 정책이 함께 적용될 수 있습니다. 이용자는 「전자상거래 등에서의
        소비자보호에 관한 법률」에 따라 결제일부터 7일 이내에 청약을 철회할 수 있습니다. 다만 디지털 콘텐츠(맞춤 리포트 등)의 제공이 이미 시작된
